...
class color_unet(nn.Module):
    def __init__(self, bn=True, drop_rate=0, classes=3):
        '''
        '''
        self.classes=classes
        super(color_unet,self).__init__()
        self.input = double_conv(1,16,bn,drop_rate)
        #begin contraction
        ...
        #begin expansion
        self.expanse1 = up_conv(128, bn, drop_rate)
        self.expanse2 = up_conv(64, bn, drop_rate)
    ...
    def forward(self, x):
        ...
        #expansion
        x = self.expanse1(x2,x3)
        x = self.expanse2(x1,x)
        x = self.expanse3(x0,x)
        x = self.out_conv(x)
